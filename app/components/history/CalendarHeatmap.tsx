...
const CalendarHeatmap: React.FC<CalendarHeatmapProps> = ({ moods }) => {
  ...
  const moodMap = useMemo(() => {
    return moods.reduce((acc, mood) => {
      ...
      // Keep the latest mood for a specific day if multiple exist
  ...
  const weekDays = ['Mon', '', 'Wed', '', 'Fri', '', 'Sun']; // GitHub style skip labels
           ...
           <div className="flex mb-2 pl-8 text-xs text-gray-400 font-medium">
                {monthLabels.map((m, i) => (
                    <div 
                        key={m.label + i} 
                        style={{ 
                            // Approximate width of a week (12px + gap) * 4.3 weeks
